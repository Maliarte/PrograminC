...
int funcaoAuxiliar(TLista listaA, TLista listaB)
{
    ...
    while (aux1 != NULL)
    {
        ...
        while (aux2 != NULL)
        {
            //verificando quantas vezes cada elemento dalistaA aparece na listaB 
...
void exibir(TLista L)
{
    ...
    else
    {
        ...
        while (aux != NULL) //'aux' percorrerá os nós da lista até que ele chegue ao valor NULL
        {
            ...
            printf("%d ", aux->valor); //lê-se: auxpontado valor
            //printf ("%d ", (*aux).valor); equivalenteao comando da linha anterior
