...
void main()
{
	...
	{
printf ("\n\nAs listas nao possuem os mesmos
elementos!");
	}
}
...
int funcaoAuxiliar(TLista listaA, TLista listaB)
{
	...
	while (aux1 != NULL)
	{
		...
		while (aux2 != NULL)
		{
			//verificando quantas vezes cada elemento da
			listaA aparece na listaB if (aux1->valor == aux2->valor)
			{
				cont++;
			}
		...
		//se o elemento da listaA não aparecer na listaB, as listas não
		possuem os mesmos elementos if (cont == 0)
		{
			return 0;
		}
		else
		{
			//continuar a busca indo para o próximo elemento
			da listaA
				cont = 0;
			aux1 = aux1->prox;
			aux2 = listaB;
...
void exibir(TLista L)
{
	...
	else
	{
		...
		aux = L;
		while (aux != NULL) 
		//'aux' percorrerá os nós da lista até que ele chegue ao valor NULL
			{
