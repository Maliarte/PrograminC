...
Considere a existência de uma lista encadeada L1, do tipo TNotas, que consiste
em um ponteiro para TNo1 com os campos matricula, disciplina e nota – além de um
ponteiro para o próximo nó da lista. Pede-se o desenvolvimento de uma função
que crie uma lista encadeada L2, do tipo Disciplinas (ponteiro para TNo2 com os
campos disciplina, media e prox).
...
typedef struct TNo1
{
	...
	int matr; // - valor: o dado armazenado na lista;
	char disciplina;
	float nota;
...
typedef TNotas *TLista; 
typedef TDisciciplina *TLista;
...
int inserir(TLista *L, int numero)
{
	...
	TLista aux = (TLista)malloc(sizeof(TNotas));
