/*
	...
	- Exemplo de uso da função 'fwrite'
*/
...
int salvar (TCarro ag[], int tamanho, char nomeArq[])
{
	...
	{
		//percorrndo o vetor de carros
		for (i=0;i<tamanho;i++)
		{
			//escrevendo todo o struct ag[i], de uma única vez
			fwrite (&ag[i], sizeof(TCarro), 1, arq);
			
			/*argumentos:
				1º: endereço a partir do qual serão obtidos os bytes que serão escritos no arquivo;
				2º: tamanho do "bloco" que será escrito no arquivo;
				3º: quantidade de blocos que serão escritos no arquivo;
				4º: arquivo para o qual serão transferidos os dados.
			*/
		}
